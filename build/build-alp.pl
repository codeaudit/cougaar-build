...
#     file: build-org.cougaar.pl
#  purpose: alp R6 build script (perl)
#  expects: 
#    usage: ./build-alp <args>
#    notes: build-org.cougaar.txt & build-org.cougaar.pod
# --------
# revision: Mar 2000  - Getting started. 
#
...
my $BUILD_CVSROOT        = "/cvs/alp/master";    # (-r <string>) CVS repository location.
...
# Check out the recursive build (AlpMake) & 'Writer' Sources. HARD-CODED location for build stuff.
check_out_sources($SRC, "/cvs/alp/master", $BUILD_BUILD_CVS_TAG, "build")        if ($BUILD_SKIP_CVS eq "FALSE");
...
# Build the build the 'Writers'
recursive_build("$SRC/build/src",  "$SRC/build/src")                if ($BUILD_SKIP_BUILD eq "FALSE");
...
create_jar($LIB, "build.jar", "$SRC/build/src")            if ($BUILD_PREFIX eq "core");
sign_jar("$LIB/build.jar", "/usr/local/etc/.keystore")     if ($BUILD_PREFIX eq "core");
...
sub parse_options() {
    ...
    getopts("r:t:T:p:b:d:v:w:W:hisSV", \%option);
...
{
    if ( ($BUILD_PREFIX eq 'core')     || 
         ($BUILD_PREFIX eq 'alpine')   || 
         ($BUILD_PREFIX eq 'server')   || 
         ($BUILD_PREFIX eq 'tools')    || 
         ($BUILD_PREFIX eq 'utility')  || 
...
sub copy_disk_overlays_to_distribution()
{
    ...
    my $cmd;
    my $overlay_root = "/build/dist/alp-overlays-MB6/";
...
sub set_classpath()
{
	...
	my @modules = ('core', 'utility', 'alpine' );
	#
	my $module;
...
sub recursive_build()
{
    ...
    chdir("$SRC/build/AlpMake") || die "ERROR!! Cannot chdir to $SRC/build.  $!";
